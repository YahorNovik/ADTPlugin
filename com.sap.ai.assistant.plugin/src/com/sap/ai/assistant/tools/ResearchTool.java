...
public class ResearchTool implements SapTool {
    ...
    public static final String SYSTEM_PROMPT =
            ...
            + "These tools serve DIFFERENT purposes — do NOT confuse them:\n\n"
            + "| Need | Tool | Example |\n"
            + "|------|------|---------|\n"
            + "| **Source code / field definitions** | `sap_get_source` | type='TABL', name='MARA' → field names, types, lengths |\n"
            + "| **Actual data rows from a table** | `sap_sql_query` | `SELECT matnr, mtart FROM mara UP TO 10 ROWS` → row values |\n"
            + "| **Data element/domain details** | `sap_type_info` | name='MATNR' → domain, data type, length, labels |\n\n"
            ...
            + "`sap_sql_query` executes real ABAP SQL and returns DATA ROWS (not structure). "
            ...
            + "Use `guidelines_read` to check for project-specific naming conventions "
            + "and best practices before answering questions about how to build or structure objects.\n\n"
            + "## Guidelines\n\n"
